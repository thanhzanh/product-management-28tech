...
module.exports.changeMulti = async (req, res) => {
    ...
    switch (typeChange) {
        case "active":
            await ArticlesCategory.updateMany( {_id: {$in: ids}}, {status: "active"} ); // thay đổi active nhìu danh mục
            req.flash("success", "Cập nhật trạng thái thành công");
            break;

        case "inactive":
            await ArticlesCategory.updateMany( {_id: {$in: ids}}, {status: "inactive"} ); // thay đổi inactive nhìu danh mục
            req.flash("success", "Cập nhật trạng thái thành công");
            break;

        case "deleted-all":
            await ArticlesCategory.updateMany( // thay đổi inactive nhìu danh mục
                {_id: {$in: ids}}, 
                {
                    deleted: true,
                    deletedAt: new Date()
        ...
        case "change-position":
            for (const item of ids) {
                ...
                await ArticlesCategory.updateOne( // Vì duyệt qua từng item có id-position rồi nên updateOne
                    {_id: id},
                    {
                        position: position
                    }
